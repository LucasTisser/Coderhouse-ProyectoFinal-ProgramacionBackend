...
test("[GET] api/productos/:non-existent-id returns product not found", async () => {
  const nonExistentProductUrl =
    "http://localhost:8080/api/productos/non-existent-id";
...
test("[PUT] api/productos/:non-existent-id return product not found or invalid body content", async () => {
  const nonExistentProductUrl =
    "http://localhost:8080/api/productos/non-existent-id";
  const body = {};
  ...
  expect(response.data).toHaveProperty(
    "error",
    "product not found or invalid body content."
  );
});
